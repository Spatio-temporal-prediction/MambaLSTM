...
class MambaLSTMPredictor(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_chans, embed_dim, depths, pre_len, drop_rate, drop_path_rate):
        ...
        self.time_embed = SinusoidalTimeEncoding(embed_dim)  # 使用周期性时间编码
        # self.se_time_fusion = SETimeFusion(time_dim=embed_dim, embed_dim=embed_dim)  # SE时间融合模块
        self.se_time_fusion = SETimeFusion(embed_dim)  # SE时间融合模块
    ...
    def forward(self, x, time_features, targets_len=1):
        ...
        time_features:(B, seq_len+1,32)
        targets_len: 需要额外预测的时间步数
        """
        if self.states != None:
            self.states = None
        ...
        # 验证时间特征长度
        assert time_features.shape[1] == inputs_len + targets_len, f"需包含输入+预测时间特征，当前收到 {time_features.shape[1]}"
        last_input = x[:, -1]  # 记录最后一个输入帧
        ...
        for i in range(6):
            x_step=x[:, i]
            t_step = time_embeding[:, i]  # 对应t=0 1 2 3 4 5的特征
        ...
        for i in range(6,inputs_len - 1):
            x_step=x[:, i]
            t_step = time_embeding[:, i]  # 对应t=6 7 8 9 10的特征
            x_step = self.se_time_fusion(x_step, t_step)
            output, self.states = self.MambaLSTM(x_step, self.states)
            outputs.append(output)

        # 第二部分：自回归预测
        t_step = time_embeding[:, inputs_len-1]  # 对应t=11的特征
